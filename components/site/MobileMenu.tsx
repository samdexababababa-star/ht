...
export function MobileMenu({ brand, categories }: { brand: string; categories: Cat[] }) {
  ...
  const drawer = open ? (
    <div className="md:hidden fixed inset-0 z-[100]">
      ...
      <aside
        className="absolute top-0 right-0 bottom-0 w-80 max-w-[88vw] p-5 flex flex-col"
        style={{
            ...
            <nav className="mt-2 space-y-0.5">
              {categories.map((c) => (
                <Link
                  ...
                  onClick={() => setOpen(false)}
                  className="flex items-center justify-between px-3 py-2.5 rounded-xl text-[14px] text-foreground/80 hover:bg-white/60 hover:text-foreground"
                >
                  <span>{c.name}</span>
